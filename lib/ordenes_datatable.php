...
$columns = array(
    ...
    array( 'db' => 'mmunicipio',   'dt' => 2 ,'formatter' => function( $d, $row ) {
    ...
    array(
        ...
        'formatter' => function( $val, $row ) {
            ...
            if($row['status'] != 'subobras' && $row['status'] != 'no_integracion' && $row['status'] != 'solicitud_subobras' && $row['status'] != 'solicitud_no_integracion'){
                $result = '<a href="../comites/ejecutora_listado.php?obra_id='.$row['id_obra'].'" class="btn btn-appb btn-primary"  title="Comités" data-toggle="tooltip" data-placement="bottom" data-original-title="Comités"><i class="fa fa-users"></i></a>';
            }
            else{
            ...
            if($row['status'] == 'nueva'){
                if($row['baja_respuesta']){
                    $result .= '<button type="button" class="btn btn-appb btn-danger" data-toggle="modal" data-target="#modalRespuestaBaja" data-id="'.$val.'" data-respuesta="'.$row['baja_respuesta'].'" title="Ver respuesta" data-toggle="tooltip" data-placement="bottom" data-original-title="Dar de baja"><i class="fa fa-caret-square-o-down"></i></button>';
                }
                else{
                    $result .= '<button type="button" class="btn btn-appb" style="background-color:#EFAB4A; border: 1px solid #E8B051;" data-toggle="modal" data-target="#modalSolicitudBaja" data-id="'.$val.'" data-avance="'.$row['avance'].'" title="Dar de baja" data-toggle="tooltip" data-placement="bottom" data-original-title="Dar de baja"><i class="fa fa-caret-square-o-down"></i></button>';
                }
            }else{
                if($row['baja_respuesta']){
                    $result .= '<button type="button" class="btn btn-appb btn-danger" data-toggle="modal" data-target="#modalRespuestaBaja" data-id="'.$val.'" data-respuesta="'.$row['baja_respuesta'].'" title="Ver respuesta" data-toggle="tooltip" data-placement="bottom" data-original-title="Dar de baja"><i class="fa fa-caret-square-o-down"></i></button>';
